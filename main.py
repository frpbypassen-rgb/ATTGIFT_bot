...
def process_radar_logic(chat_id, raw_codes_list, product_info=None):
    ...
    try:
        ...
        bot.send_message(chat_id, f"⚠️ تم تحديث المخزون بنجاح، لكن حدث خطأ أثناء إرسال ملفات الفرز.")
...
def handle_contact_sharing(msg):
    ...
    if u.get("status") == "frozen":
        bot.send_message(uid, "✅ تم استلام بياناتك بنجاح.\nحسابك الآن قيد المراجعة، يرجى الانتظار لحين التفعيل.", reply_markup=types.ReplyKeyboardRemove())
...
def handle_client_reports(call):
    ...
    if call.data == "client_purchases":
        ...
        if not history: return bot.answer_callback_query(call.id, "لا توجد مشتريات مسجلة.", show_alert=True)
        txt = "🛒 **آخر 10 مشتريات لك:**\n\n"
        for t in history:
            txt += f"▪️ {t.get('date', '')} | הפاتورة #{t.get('order_id', 'N/A')} | {t.get('item_name', 'منتج')} (x{t.get('quantity', 1)}) | السعر: {round(float(t.get('price', 0)), 2)}\n"
...
def process_card_charging(msg):
    if msg.text in MENU_BUTTONS:
        bot.send_message(msg.chat.id, "🔄 تم إلغاء العملية. يرجى الضغط على الزر المطلوب مجدداً.")
        return
    ...
    if not card:
        ...
        if attempts >= 5:
            ...
            bot.send_message(uid, "🚫 تم إيقاف حسابك لتكرار إدخال أكواد خاطئة.")
        else:
...
def handle_subcategory_selection(call):
    ...
    cat_name = parts[1]; sub_name = parts[2]
...
def finalize_purchase(msg, user_data, item_ref, available_qty):
    if msg.text in MENU_BUTTONS:
        bot.send_message(msg.chat.id, "🔄 تم إلغاء عملية الشراء. يرجى الضغط على الزر المطلوب مجدداً.")
        return
    ...
    try:
        ...
        if requested_qty < 10 or requested_qty % 10 != 0:
            return bot.send_message(uid, "❌ الكمية يجب أن تكون من مضاعفات الـ 10 (10، 20، 50...). تم إلغاء الطلب.")
    except ValueError:
        return bot.send_message(uid, "❌ يرجى إدخال أرقام صحيحة فقط. تم إلغاء الطلب.")
        
    if requested_qty > available_qty:
        return bot.send_message(uid, f"❌ الكمية المطلوبة غير متوفرة. المتاح حالياً: {available_qty}")
    ...
    if user_balance < total_cost:
        return bot.send_message(uid, f"❌ رصيدك غير كافي لإتمام العملية.\nالمطلوب: {total_cost}\nرصيدك الحالي: {user_balance}")
    ...
    if len(batch) < requested_qty:
        return bot.send_message(uid, "❌ حدث خطأ في توافر الأكواد بسبب سحب آخر. يرجى المحاولة مرة أخرى.")
    ...
    if SHEET_WEBHOOK_URL and SHEET_WEBHOOK_URL.startswith("http"):
        try:
            requests.post(SHEET_WEBHOOK_URL, json={"order_id": order_id, "date": dt_str, "phone": user_data.get('phone'), "item_name": f"{product_name} (x{requested_qty})", "price": total_cost}, timeout=3)
        except Exception: pass
...
def admin_find_and_render_customer(msg):
    if msg.text in MENU_BUTTONS:
        bot.send_message(msg.chat.id, "🔄 تم الإلغاء. يرجى الضغط على الزر مجدداً.")
        return
...
def handle_tier_setting(call):
    ...
    bot.answer_callback_query(call.id, f"تم تغيير مستوى العميل إلى {parts[2]}")
...
def admin_add_product(msg):
    ...
    try:
        ...
        doc = io.BytesIO(file_bytes); doc.name = "Template_Products.xlsx"
...
def admin_process_manual_codes(msg):
    ...
    if not p_info: return bot.send_message(msg.chat.id, "❌ انتهت الجلسة.")
...
def admin_finalize_price_update(msg):
    if msg.text in MENU_BUTTONS:
        bot.send_message(msg.chat.id, "🔄 تم الإلغاء. يرجى الضغط على الزر مجدداً.")
        return
    if not msg.document or not msg.document.file_name.endswith('.xlsx'): return bot.send_message(msg.chat.id, "❌ يرجى رفع ملف إكسيل .xlsx")
...
def manage_stock_cmd(msg):
    ...
    ask_msg = bot.send_message(msg.chat.id, text + "\n👉 أرسل **اسم المنتج** لإدارته:", parse_mode="Markdown")
...
def admin_update_stock_price(msg, name):
    if msg.text in MENU_BUTTONS:
        ...
        return
    try:
        parts = msg.text.split(":"); p1 = float(parts[0].strip())
...
def admin_delete_stock_qty(msg, name):
    if msg.text in MENU_BUTTONS:
        ...
        return
    try:
        docs = list(stock.find({"name": name, "sold": False}).limit(int(msg.text.strip())))
        if not docs: return bot.send_message(msg.chat.id, "❌ لا يوجد رصيد متاح للحذف.")
        res = stock.delete_many({"_id": {"$in": [d['_id'] for d in docs]}})
...
def execute_generate_cards(msg):
    ...
    try:
        parts = msg.text.split(":")
        count, val = int(parts[0].strip()), round(float(parts[1].strip()), 2)
        arr = []
...
def bot_polling_worker():
    """عزل البوت عن الويب سيرفر لمنع إيقافه من المنصة"""
    try:
        bot.remove_webhook()
    except: pass
