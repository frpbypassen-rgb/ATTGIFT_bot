...
def process_radar_logic(chat_id, raw_codes_list, product_info=None):
    ...
    # ⚠️ التعديل الجذري: إرسال الملفات كـ Tuple
    if len(accepted_docs) > 0:
        file_acc = generate_simple_excel(accepted_docs, "Accepted_New_Codes")
        bot.send_document(chat_id, ("Accepted_Codes.xlsx", file_acc.getvalue()), caption="📁 ملف الأكواد التي تم إضافتها بنجاح.")
        
    if len(rejected_docs) > 0:
        file_rej = generate_simple_excel(rejected_docs, "Rejected_Duplicates")
        bot.send_document(chat_id, ("Rejected_Duplicates.xlsx", file_rej.getvalue()), caption="⚠️ ملف الأكواد المكررة (التي تم استبعادها ومسحها).")
...
def show_account_info(msg):
    ...
    kb.add(types.InlineKeyboardButton("🛒 سجل المشتريات", callback_data="client_purchases"),
...
def handle_client_reports(call):
    ...
    if call.data == "client_purchases":
        ...
        for t in history:
            txt += f"▪️ {t.get('date', '')} | الفاتورة #{t.get('order_id', 'N/A')} | {t.get('item_name', 'منتج')} (x{t.get('quantity', 1)}) | السعر: {t.get('price', 0)}\n"
        ...
        for t in history:
            if t.get('type') == "شراء": txt += f"🔴 خصم | {t.get('date', '')} | شراء {t.get('item_name', '')} | -{t.get('price', 0)}\n"
            else: txt += f"🟢 إضافة | {t.get('date', '')} | {t.get('type', '')} | +{t.get('amount', 0)}\n"
                
    bot.send_message(uid, txt, parse_mode="Markdown")
    bot.answer_callback_query(call.id)

# =======================================================
# 8. الشحن والتسوق (Charging & Shopping)
# =======================================================
...
def finalize_purchase(msg, user_data, item_ref, available_qty):
    ...
    transactions.insert_one({
        ...
        "phone": user_data.get("phone"), "type": "شراء", "item_name": product_name, 
    ...
    for d in batch:
        purchased_items_data.append({
            'code': d['code'], 'serial': d.get('serial', ''), 
            'pin': d.get('pin', ''), 'op_code': d.get('op_code', '')
        })
        
    file_stream = generate_customer_excel_file(purchased_items_data, order_id, dt_str, product_name)
    
    # ⚠️ التعديل الجذري: إرسال الملف كـ Tuple
    bot.send_document(uid, (f"Invoice_{order_id}.xlsx", file_stream.getvalue()), caption=f"📁 فاتورة الأكواد | رقم #{order_id}")
    ...
    for admin_id in get_all_admins():
        try:
            ...
            bot.send_document(admin_id, (f"Admin_Inv_{order_id}.xlsx", file_stream.getvalue()), caption=f"📁 نسخة فاتورة #{order_id}")
        except Exception: pass
...
def admin_invoices_log(msg):
    ...
    try:
        ...
        # إزالة Markdown من هنا أيضاً لزيادة الأمان
...
def admin_render_customer_card(uid, chat_id, message_id=None):
    ...
    if stat == "active":
        kb.add(types.InlineKeyboardButton("❄️ تجميد الحساب", callback_data=f"adm_freeze_{uid}"), 
    ...
    kb.add(types.InlineKeyboardButton("🎚️ تغيير مستوى الأسعار", callback_data=f"adm_chgtier_{uid}"), 
...
def handle_admin_customer_actions(call):
    ...
    if action == "report":
        ...
        bot.send_message(call.message.chat.id, txt) # بدون ماركدوان لحماية الرسالة
        bot.answer_callback_query(call.id)
        return

    if action == "chgtier":
        kb = types.InlineKeyboardMarkup(row_width=3)
        kb.add(types.InlineKeyboardButton("المستوى 1 🥉", callback_data=f"set_tier_1_{uid}"),
...
def admin_add_product(msg):
    ...
    txt = (
        "➕ **إضافة منتجات للمخزن:**\n\n"
        "📁 **الطريقة الأولى:** قم بتحميل قالب الإكسيل المرفق، املأه بالأكواد وأعد رفعه.\n"
    ...
    try:
        ...
        # ⚠️ التعديل الجذري: إرسال الملف كـ Tuple
        ask_msg = bot.send_document(msg.chat.id, ("Template_Products.xlsx", template_file.getvalue()), caption=txt, parse_mode="Markdown")
...
def admin_process_product_input(msg):
    ...
    if msg.document:
            ...
            for i, row in enumerate(ws.iter_rows(values_only=True)):
                ...
                try:
                    ...
                    batch_data.append({
                        ...
                        "code": code_val, 
                        "serial": str(row[7]).strip() if len(row) > 7 and row[7] else "", 
                        "pin": str(row[8]).strip() if len(row) > 8 and row[8] else "", 
                        "op_code": str(row[9]).strip() if len(row) > 9 and row[9] else ""
                    })
        ...
        try:
            ...
            product_info = {
                "cat": parts[0].strip(), "sub": parts[1].strip(), "name": parts[2].strip(), 
                "price_1": p1, "price_2": p2, "price_3": p3
            }
...
def admin_process_manual_codes(msg):
    ...
    if msg.text:
        ...
        for line in lines:
            ...
            batch_data.append({
                "code": code_val, "serial": serial_val,
                "pin": pin_val, "op_code": op_val
            })
...
def admin_bulk_price_edit(msg):
    ...
    pipeline = [
        {"$match": {"sold": False}}, 
        {"$group": {
            "_id": "$name", "cat": {"$first": "$category"}, "sub": {"$first": "$subcategory"}, 
            "p1": {"$first": "$price_1"}, "p2": {"$first": "$price_2"}, "p3": {"$first": "$price_3"}
        }}
    ]
    ...
    for p in prods:
        ws.append([p["_id"], p.get("cat", ""), p.get("sub", ""), p.get("p1", 0), p.get("p2", 0), p.get("p3", 0)])
    ...
    ask_msg = bot.send_document(
        msg.chat.id, 
        ("Prices_Update_List.xlsx", stream.getvalue()), 
...
def admin_execute_report(uid, rep_type, start_date, end_date):
    ...
    if rep_type == "all":
        ...
        # ⚠️ التعديل الجذري: إرسال الملف كـ Tuple
        bot.send_document(uid, ("Report_All_Operations.xlsx", file_stream.getvalue()), caption="✅ تم تجهيز التقرير الشامل.")
...
def admin_execute_single_report(msg, filtr):
    ...
    client_phone = u.get('phone', 'Client')
    # ⚠️ التعديل الجذري: إرسال الملف كـ Tuple
    bot.send_document(msg.chat.id, (f"Report_{client_phone}.xlsx", file_stream.getvalue()), caption=f"✅ تقرير مخصص للعميل: {safe_str(u.get('name'))}")
...
# =======================================================
# 12. محرك تشغيل السيرفر (Render Setup)
# =======================================================
