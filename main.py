...
# 🟢 نظام الفحص الذاتي لقاعدة البيانات
try:
    ...
    client = MongoClient(
        ...
        maxPoolSize=50, 
        serverSelectionTimeoutMS=5000 # سيُظهر خطأ إذا لم يتصل خلال 5 ثوانٍ
    )
    # اختبار الاتصال الفعلي
    client.admin.command('ping')
    print("✅ تم الاتصال بقاعدة البيانات (MongoDB) بنجاح!")
except Exception as e:
    print(f"❌ فشل الاتصال بقاعدة البيانات! يرجى التأكد من إعدادات Network Access في MongoDB: {e}")
...
try:
    ...
    print(f"❌ خطأ في تهيئة البوت (تأكد من صحة التوكن): {e}")
...
def process_radar_logic(chat_id, raw_codes_list, product_info=None):
    if not raw_codes_list: return bot.send_message(chat_id, "❌ لم يتم العثور على أي بيانات صالحة لمعالجتها.")
    ...
    bot.send_message(chat_id, f"📊 **تقرير معالجة المخزون:**\n\n✅ أكواد جديدة مقبولة: `{len(accepted_docs)}`\n❌ أكواد مكررة مرفوضة: `{len(rejected_docs)}`", parse_mode="Markdown")
    
    try:
        if len(accepted_docs) > 0:
            doc_acc = io.BytesIO(generate_simple_excel(accepted_docs)); doc_acc.name = "Accepted_New_Codes.xlsx"
            bot.send_document(chat_id, doc_acc, caption="📁 ملف الأكواد التي تم إضافتها بنجاح.")
        if len(rejected_docs) > 0:
            doc_rej = io.BytesIO(generate_simple_excel(rejected_docs)); doc_rej.name = "Rejected_Duplicates.xlsx"
            bot.send_document(chat_id, doc_rej, caption="⚠️ ملف الأكواد المكررة (التي تم استبعادها).")
    except Exception as e:
        bot.send_message(chat_id, f"⚠️ تم التحديث بنجاح، لكن حدث خطأ أثناء إرسال ملفات الفرز: {e}")
...
def process_card_charging(msg):
    if msg.text in MENU_BUTTONS: return bot.send_message(msg.chat.id, "تم إلغاء عملية الشحن.")
    uid = msg.chat.id
    u = users.find_one({"_id": uid})
...
def handle_subcategory_selection(call):
    ...
    if total_count < 10: return bot.answer_callback_query(call.id, "⚠️ الكمية المتبقية أقل من الحد الأدنى للشراء (10 أكواد).", show_alert=True)
        
    sample_item = available_items[0]
    p1 = float(sample_item.get("price_1", sample_item.get("price", 0)))
    final_price = round(p1 if tier == 1 else (float(sample_item.get("price_2", p1)) if tier == 2 else float(sample_item.get("price_3", p1))), 2)
    
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("🛒 تأكيد طلب شراء", callback_data=f"buy_{sample_item['_id']}"))
    bot.send_message(uid, f"📦 المنتج: **{sample_item['name']}**\n💰 السعر: **{final_price}**\n📊 الكمية المتوفرة: {total_count}\n\n⚠️ أقل كمية للطلب هي 10 ومضاعفاتها.", reply_markup=kb, parse_mode="Markdown")
...
def finalize_purchase(msg, user_data, item_ref, available_qty):
    if msg.text in MENU_BUTTONS: return bot.send_message(msg.chat.id, "تم إلغاء عملية الشراء.")
    uid = msg.chat.id
    ...
    try:
        ...
        doc_user = io.BytesIO(generate_customer_excel_file(purchased_items_data, order_id, dt_str, product_name))
        ...
        for admin_id in get_all_admins():
            try:
                doc_admin = io.BytesIO(generate_customer_excel_file(purchased_items_data, order_id, dt_str, product_name))
        ...
        bot.send_message(uid, "⚠️ تمت العملية بنجاح وتم حفظ مشترياتك في النظام، لكن حدث خطأ أثناء إرسال الإكسيل. يرجى التواصل مع الإدارة.")
...
def admin_render_customer_card(uid, chat_id, message_id=None):
    ...
    stat_label = "محظور 🚫" if u.get("status") == "blocked" else ("نشط ✅" if u.get("status") == "active" else "مجمد ❄️")
    
    info = (f"👤 **ملف بيانات العميل:**\n\n📛 الاسم: {safe_str(u.get('name'))}\n🆔 ID: `{uid}`\n📱 الهاتف: `{u.get('phone')}`\n💰 الرصيد: **{round(float(u.get('balance',0)), 2)}**\n🎚️ المستوى الحالي: **{tier_label}**\nحالة الحساب: **{stat_label}**")
    kb = types.InlineKeyboardMarkup(row_width=2)
    if u.get("status") == "active": kb.add(types.InlineKeyboardButton("❄️ تجميد الحساب", callback_data=f"adm_freeze_{uid}"), types.InlineKeyboardButton("🚫 حظر و طرد", callback_data=f"adm_block_{uid}"))
    elif u.get("status") == "frozen": kb.add(types.InlineKeyboardButton("✅ تفعيل الحساب", callback_data=f"adm_activate_{uid}"), types.InlineKeyboardButton("🚫 حظر و طرد", callback_data=f"adm_block_{uid}"))
...
def handle_tier_setting(call):
    parts = call.data.split("_")
    users.update_one({"_id": int(parts[3])}, {"$set": {"tier": int(parts[2])}})
    bot.answer_callback_query(call.id, f"تم تغيير المستوى إلى {parts[2]}")
    admin_render_customer_card(int(parts[3]), call.message.chat.id, call.message.message_id)

@bot.message_handler(func=lambda m: m.text == "➕ منتج")
def admin_add_product(msg):
    if not is_admin(msg.chat.id): return
    txt = ("➕ **إضافة منتجات للمخزن:**\n\n📁 **الطريقة الأولى:** رفع ملف الإكسيل المرفق.\n✍️ **الطريقة الثانية:** أرسل `القسم:الفئة:الاسم:السعر1:السعر2:السعر3`")
    try:
        doc = io.BytesIO(generate_products_template()); doc.name = "Template_Products.xlsx"
...
def admin_process_product_input(msg):
    ...
    if msg.document:
        ...
        bot.send_message(msg.chat.id, "⏳ جاري قراءة البيانات...")
        try:
            ...
            for i, row in enumerate(ws.iter_rows(values_only=True)):
                ...
                try:
                    ...
                    if not code_val: errors_count += 1; continue
                    batch_data.append({
                        ...
                        "code": code_val, "serial": str(row[7]).strip() if len(row) > 7 and row[7] else "", "pin": str(row[8]).strip() if len(row) > 8 and row[8] else "", "op_code": str(row[9]).strip() if len(row) > 9 and row[9] else ""
                    })
                except: errors_count += 1
...
def admin_bulk_price_edit(msg):
    ...
    if not prods: return bot.send_message(msg.chat.id, "❌ لا توجد منتجات متاحة للبيع في المخزن لتعديل أسعارها.")
...
def admin_finalize_price_update(msg):
    if msg.text in MENU_BUTTONS: return bot.send_message(msg.chat.id, "🔄 تم الإلغاء.")
    if not msg.document or not msg.document.file_name.endswith('.xlsx'): return bot.send_message(msg.chat.id, "❌ يرجى رفع ملف الإكسيل بصيغة .xlsx")
    bot.send_message(msg.chat.id, "⏳ جاري قراءة الملف وتحديث الأسعار...")
    try:
        wb = openpyxl.load_workbook(io.BytesIO(bot.download_file(bot.get_file(msg.document.file_id).file_path))); ws = wb.active
        updated_count = 0
        for i, row in enumerate(ws.iter_rows(values_only=True)):
            if i == 0 or not row[0]: continue
            try:
                p1 = float(row[3]) if row[3] is not None else 0
                res = stock.update_many({"name": str(row[0]).strip(), "sold": False}, {"$set": {"price_1": p1, "price_2": float(row[4]) if row[4] is not None else p1, "price_3": float(row[5]) if row[5] is not None else p1}})
                if res.modified_count > 0: updated_count += 1
            except Exception: continue
        bot.send_message(msg.chat.id, f"✅ تمت العملية بنجاح. تم تحديث أسعار `{updated_count}` نوع من المنتجات.")
    except Exception as e: bot.send_message(msg.chat.id, f"❌ حدث خطأ:\n{e}")
...
def show_stock_item_panel(msg):
    ...
    text = (f"📦 المنتج: `{name}`\n💰 الأسعار الحالية:\nمستوى 1: {p1} | مستوى 2: {item.get('price_2', p1)} | مستوى 3: {item.get('price_3', p1)}\n📊 الكمية المتوفرة: {stock.count_documents({'name': name, 'sold': False})}")
...
def admin_update_stock_price(msg, name):
    ...
    try:
        ...
        stock.update_many({"name": name, "sold": False}, {"$set": {"price_1": p1, "price_2": float(parts[1].strip()) if len(parts) > 1 else p1, "price_3": float(parts[2].strip()) if len(parts) > 2 else p1}})
...
def admin_delete_stock_qty(msg, name):
    if msg.text in MENU_BUTTONS: return bot.send_message(msg.chat.id, "🔄 تم الإلغاء.")
    try:
        docs_to_delete = list(stock.find({"name": name, "sold": False}).limit(int(msg.text.strip())))
        if not docs_to_delete: return bot.send_message(msg.chat.id, "❌ لا يوجد رصيد متاح للحذف.")
        res = stock.delete_many({"_id": {"$in": [d['_id'] for d in docs_to_delete]}})
...
def admin_report_finalize(call):
    uid = call.message.chat.id; mode = call.data.split("_")[1]
...
def admin_process_date_input(msg, rep_type):
    ...
    try:
        parts = msg.text.split(":")
        admin_execute_report(msg.chat.id, rep_type, parts[0].strip(), parts[1].strip())
...
def admin_execute_single_report(msg, filtr):
    ...
    except Exception as e: bot.send_message(msg.chat.id, f"❌ حدث خطأ: {e}")
...
def bot_polling_worker():
    """عزل البوت عن الويب سيرفر لمنع إيقافه من المنصة وتمنعه من الانهيار"""
    try:
        bot.remove_webhook()
    except: pass
